...
"""
  Author:   --<v1ll4n>
  Purpose: Provide plugins and linkages some useful utils
  Created: 2016/10/29
"""
import uuid
import time
...
class Pool(object):
    ...
    def _worker(self, dictobj):
        ...
        args = dictobj['args']
        argv = dictobj['argv']
        #sig = blinker.signal(self.signal_name)
        result = func(*args, **argv)
        #sig.send('anonymous', result=result)
        self.lock.acquire()
    ...
    def _add_result_to_queue(self, **kw):
        """"""
        assert kw.has_key('result'), '[!] blinker.signal send Result Error!'
    ...
    def _init_signal(self):
        '''Bind the signal to callback_func'''
        sig = blinker.signal(self.signal_name)
        sig.connect(self._add_result_to_queue)
